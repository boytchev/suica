...
console.log(`Suica 2.0.11 (220203)`);const DEBUG_CALLS=!false;class Suica
...
static DEFAULT={BACKGROUND:{COLOR:'whitesmoke'},ORIENTATION:'XYZ',SIZE:'30',OXYZ:{COLOR:'black',SIZE:30},DEMO:{DISTANCE:100,ALTITUDE:30},ONTIME:{SRC:null},POINT:{CENTER:[0,0,0],COLOR:'crimson',SIZE:7},CUBE:{CENTER:[0,0,0],COLOR:'cornflowerblue',FRAMECOLOR:'black',SIZE:30},SQUARE:{CENTER:[0,0,0],COLOR:'cornflowerblue',FRAMECOLOR:'black',SIZE:30},}
...
createMaterials()
{var CANVAS_SIZE=64;var canvas=document.createElement('canvas');canvas.width=CANVAS_SIZE;canvas.height=CANVAS_SIZE;var context=canvas.getContext('2d');context.fillStyle='white';context.beginPath();context.arc(CANVAS_SIZE/2,CANVAS_SIZE/2,CANVAS_SIZE/2-1,0,2*Math.PI);context.fill();Suica.pointMaterial=new THREE.PointsMaterial({color:'white',size:5,sizeAttenuation:true,map:new THREE.CanvasTexture(canvas),transparent:true,alphaTest:0.75,});Suica.solidMaterial=new THREE.MeshStandardMaterial({color:'cornflowerblue',side:THREE.DoubleSide,});Suica.lineMaterial=new THREE.LineBasicMaterial({color:'black',});}
...
{suica.parser?.parseTags();if(DEBUG_CALLS)console.log(`:: ${suica.id}.point(${center},${size},${color})`);super(suica,THREE.Points,Point.geometry,Suica.pointMaterial.clone());this.center=center;this.color=color;this.size=size;suica.scene.add(this.threejs);}
get size()
...
{suica.parser?.parseTags();if(DEBUG_CALLS)console.log(`:: ${suica.id}.square(${center},${size},${color})`);super(suica,THREE.Mesh,Square.geometry,Suica.solidMaterial.clone());this.center=center;this.color=color;this.size=size;suica.scene.add(this.threejs);}}
...
{suica.parser?.parseTags();if(DEBUG_CALLS)console.log(`:: ${suica.id}.squareFrame(${center},${size},${color})`);super(suica,THREE.LineSegments,SquareFrame.geometry,Suica.lineMaterial.clone());this.center=center;this.color=color;this.size=size;suica.scene.add(this.threejs);}}
...
{suica.parser?.parseTags();if(DEBUG_CALLS)console.log(`:: ${suica.id}.cube(${center},${size},${color})`);super(suica,THREE.Mesh,Cube.geometry,Suica.solidMaterial.clone());this.center=center;this.color=color;this.size=size;suica.scene.add(this.threejs);}}
class CubeFrame extends Mesh
{static geometry=new THREE.EdgesGeometry(Cube.geometry);constructor(suica,center,size,color)
{suica.parser?.parseTags();if(DEBUG_CALLS)console.log(`:: ${suica.id}.cubeFrame(${center},${size},${color})`);super(suica,THREE.LineSegments,CubeFrame.geometry,Suica.lineMaterial.clone());this.center=center;this.color=color;this.size=size;suica.scene.add(this.threejs);}}
