...
console.log(`Suica 2.0.3 (220122)`);const DEBUG_CALLS=!false;class Suica
{static current;static allSuicas=[];static DEFAULT={BACKGROUND:{COLOR:'whitesmoke'},OXYZ:{COLOR:'black',SIZE:30},DEMO:{DISTANCE:100,ALTITUDE:30},ONTIME:{SRC:null},POINT:{CENTER:[0,0,0],COLOR:'crimson',SIZE:7},CUBE:{CENTER:[0,0,0],COLOR:'cornflowerblue',SIZE:30},}
...
if(DEBUG_CALLS)console.log(`Suica :: ${this.id}`);this.suicaTag=suicaTag;this.createCanvas();this.createRenderer();this.createMaterials();this.parser=new HTMLParser(this);this.onTimeHandler=null;this.demoViewPoint=null;Suica.current=this;Suica.allSuicas.push(this);window[this.id]=this;}
createCanvas()
...
createRenderer()
{this.renderer=new THREE.WebGLRenderer({canvas:this.canvas,alpha:true,antialias:true});this.scene=new THREE.Scene();var color=getComputedStyle(this.suicaTag).backgroundColor;if(color=='rgba(0, 0, 0, 0)')
{color=this.suicaTag.getAttribute('background')||Suica.DEFAULT.BACKGROUND.COLOR;}
this.scene.background=Suica.parseColor(color);this.camera=new THREE.PerspectiveCamera(40,this.canvasAspect,1,1000);this.camera.position.set(0,0,100);this.camera.lookAt(this.scene.position);this.light=new THREE.PointLight('white',1);this.light.position.set(100,150,300);this.scene.add(this.light);var that=this;this.lastTime=0;function loop(time)
{time/=1000;if(that.demoViewPoint)
{that.camera.position.setFromCylindricalCoords(that.demoViewPoint.distance,time,that.demoViewPoint.altitude);that.camera.lookAt(that.scene.position);that.light.position.copy(that.camera.position);}
...
{center=center.split(',');return[Number(center[0]),Number(center[1]),Number(center[2])];}}
return center;}
...
class Drawing
{static current;constructor(width=32,height=width)
{this.canvas=document.createElement('canvas');this.canvas.width=width;this.canvas.height=height;this.texture=null;this.context=this.canvas.getContext('2d');this.context.scale(1,-1);this.context.translate(0,-height);this.context.clearRect(0,0,width,height);this.context.beginPath();}
moveTo(x=0,y=0)
...
{Drawing.current=new Drawing(width,height);return Drawing.current;}
...
{var p=suica.cube(elem.getAttribute('center')||Suica.DEFAULT.CUBE.CENTER,elem.getAttribute('size')||Suica.DEFAULT.CUBE.SIZE,elem.getAttribute('color')||Suica.DEFAULT.CUBE.COLOR);if(elem.hasAttribute('x'))p.x=Number(elem.getAttribute('x'));if(elem.hasAttribute('y'))p.y=Number(elem.getAttribute('y'));if(elem.hasAttribute('z'))p.z=Number(elem.getAttribute('z'));var id=elem.getAttribute('id');if(id)window[id]=p;}}
﻿
...
{this.material.map=drawing.image;return;}
...
{suica.parser?.parseTags();if(DEBUG_CALLS)console.log(`:: ${suica.id}.cube(${center},${size},${color})`);super(Cube.geometry,Suica.solidMaterial.clone());this.suica=suica;this.center=center;this.color=color;this.size=size;suica.scene.add(this);}
get size()
{this.suica.parser?.parseTags();return this.scale.x;}
set size(size)
{this.suica.parser?.parseTags();this.scale.set(size,size,size);}}
