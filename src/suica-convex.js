...
import { cloneEvents, parseColor, parseSize } from './suica-globals.js';
...
class Convex extends Mesh {
	...
	constructor( suica, points, size, color ) {
		...
		this.center = [ 0, 0, 0 ];
		this.size = parseSize( size, Convex.SIZE );
		this.color = parseColor( color, Convex.COLOR );
		this._points = points;
	...
	static generateGeometry( points ) {
		...
		var geometry = new ConvexGeometry( threejsPoints );
