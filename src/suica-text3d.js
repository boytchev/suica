...
import { cloneEvents, onLoad, parseCenter, parseColor, parseSize } from './suica-globals.js';
...
class Text3D extends Mesh {
	...
	constructor( suica, text, fontName, center, size, color ) {
		...
		this.size = parseSize( size, Text3D.SIZE );
		this.color = parseColor( color, Text3D.COLOR );
	...
	regenerateGeometry( ) {
		...
		this.solidMesh.geometry = new TextGeometry( this.text, {
			font: this._font,
			size: 1,
			depth: 1,
			curveSegments: 8,
